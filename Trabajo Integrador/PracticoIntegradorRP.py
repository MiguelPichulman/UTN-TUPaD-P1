...
def busqueda_lineal(lista, elemento):
    inicio = time.perf_counter()  # Iniciar medicion del tiempo
...
def busqueda_binaria(lista, elemento):
    ...
    while indice <= fin:
        ...
        if lista[medio] == elemento:
            fin_tiempo = time.perf_counter()
            tiempo = (fin_tiempo - inicio) * 1000  # Tiempo en milisegundos
            return medio, tiempo #el elemnto esta en la mitad
...
inventario = [random.randint(100, 999) for i in range(100)]

print("Inventario antes de ordenar:", inventario)
burbuja(inventario) #llama a funcion de ordenamiento
...
while True:
    ...
    if indice_binaria != -1:
        print(f"El producto con codigo {producto_a_buscar} esta en la posición {indice_binaria} y se demoro {tiempo_binaria:.5f} ms usando busqueda binaria.")
    else:
